            ...
            <div class="mx-auto max-w-7xl">
                <div class="grid grid-cols-1 gap-6 md:grid-cols-12">
                    <div class="md:col-span-4">
                        <div class="sticky top-8 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-md">
                            ...
                            <div class="max-h-[36rem] space-y-2 overflow-y-auto p-4">
                                @forelse($sessions as $s)
                                    <div class="flex items-start justify-between gap-2">
                                        <button
                                            wire:click="selectSession({{ $s->id }})"
                                            class="flex-1 rounded-lg p-3 text-left text-sm font-medium transition hover:bg-gray-100
                                        ...
                                        >
                                            ...
                                            <div class="mt-1 text-xs text-gray-500">{{ $s->created_at->format('M d, H:i') }}</div>
                                        </button>
                                        <button wire:click="deleteSession({{ $s->id }})" type="button" class="ml-2 text-red-500 hover:text-red-700" title="Delete chat">&times;</button>
                                    </div>
                                @empty
                                    <div class="p-3 text-sm italic text-gray-500">No chats yet. Start a new one!</div>
                                @endforelse
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="md:col-span-8">
                        <div class="flex h-96 flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-md">
                            <div class="flex items-center justify-between border-b border-slate-200 bg-gradient-to-r from-indigo-50 via-violet-50 to-cyan-50 p-4">
                                <div>
                                    ...
                                    @if($session)
                                        <p class="mt-1 text-xs text-gray-600">Started {{ $session->created_at->format('M d, Y at H:i') }}</p>
                                    @endif
                                </div>
                            </div>

                            <div class="flex-1 overflow-y-auto bg-gray-50 p-4" id="chat-messages-full">
                                @if($chatError)
                                ...
                                @elseif($session)
                                    <div class="py-12 text-center text-gray-500">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto mb-3 size-12 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
