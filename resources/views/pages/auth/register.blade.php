    ...
    <div class="flex flex-col gap-6">
        ...
        <form method="POST" action="{{ route('register.store') }}" class="flex flex-col gap-6">
            ...
            <flux:input name="email" :label="__('Email address')" :value="old('email')" type="email" required
                autocomplete="email" placeholder="email@quizforge.app" />

            <!-- Role -->
            <flux:field>
                <flux:label>I am a…</flux:label>
                <flux:radio.group name="role" :value="old('role', 'student')" class="flex flex-row gap-6">
                    <flux:radio value="student" label="Student" />
                    <flux:radio value="teacher" label="Teacher" />
                </flux:radio.group>
